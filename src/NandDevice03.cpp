...
NandDevice03::NandDevice03(void) { vfSpace = NULL; }
...
RV NandDevice03::ReadPage(BLOCK_ID blockID, PAGE_ID pageID, BYTE *buffer,
  ...
  // read process
  int blockSize = this->info.pageSize * this->info.pageCountPerBlock;
  memcpy(buffer, vfSpace + blockID * blockSize + pageID * info.pageSize,
         info.pageSize);

  readCounter[PAGE_INDEX(blockID, pageID)]++;
  readLatencyTotal += this->info.readTime.randomTime +
                      this->info.readTime.serialTime * this->info.pageSize;
...
RV NandDevice03::WritePage(BLOCK_ID blockID, PAGE_ID pageID, const BYTE *buffer,
  ...
  // write process
  int blockSize = this->info.pageSize * this->info.pageCountPerBlock;
  for (int i = 0, pos = blockID * blockSize + pageID * info.pageSize;
       i < info.pageSize; i++, pos++) {
