...
public:
	...
	VFD_INFO(IDM_VFD _id, int bc, int s1, int s2, int pcpb, int el, int rt, int st, int pt, int et) {
		id = _id;
		blockCount = bc; pageSize.size1 = s1; pageSize.size2 = s2; pageCountPerBlock = pcpb;
		eraseLimitation = el; readTime.randomTime = rt; readTime.serialTime = st;
