...
RV NandDevice04::Initialize(const VFD_INFO &info) {
  ...
  readTime =
      info.readTime.randomTime + info.readTime.serialTime * this->info.pageSize;
  ...
  for (int i = 0; i < this->info.blockCount * this->info.pageCountPerBlock;
       i++) {
 ...
 */
RV NandDevice04::ReadPage(BLOCK_ID blockID, PAGE_ID pageID, BYTE *buffer,
                          int offset, int size) {
  ASSERT(eraseCounter);
  ASSERT(blockID >= 0 && blockID < info.blockCount);
 ...
 */
RV NandDevice04::WritePage(BLOCK_ID blockID, PAGE_ID pageID, const BYTE *buffer,
                           int offset, int size) {
  ASSERT(eraseCounter);
  ASSERT(blockID >= 0 && blockID < info.blockCount);
  ASSERT(pageID >= 0 && pageID < info.pageCountPerBlock);
