...
/* Query Interface */
RV FTL01::QueryInterface(const IID &iid, void **ppv) {
  if (iid == IID_IUNKNOWN) {
...
RV FTL01::Initialize(const FTL_INFO &info, const IVFD *device) {
  Release();
  ...
  reserved =
      vfdInfo.blockCount - 1;  // set the last block as the reserved block
  ...
  for (int i = 0; i < vfdInfo.blockCount; i++) {
    ...
    blockState[i].livePages = blockState[i].deadPages =
        blockState[i].allocatedPages = 0;
...
/* Allocate a new page */
int FTL01::AllocPage(int count, LBA *lbas) {
  ASSERT(lbas);
  ASSERT(count > 0);
  ...
  for (int j = 0; j < info.mapListSize; j++) {
    ...
    if (pba.blockID == -1 || pba.pageID == -1) {  // empty lba
      ...
      if (pba.blockID == -1 || pba.pageID == -1) {  // no more space free
        for (int i = allocated; i < count; i++) lbas[i] = -1;
        return allocated;
      }
...
RV FTL01::ReadPage(LBA lba, BYTE *buffer, int offset, size_t size) {
  ...
  if (pba.blockID == -1 || pba.pageID == -1) return RV_ERROR_INVALID_LBA;

  if (flashDevice->GetFlashType() == NAND) {  // NAND
    BYTE *_buffer = new BYTE[flashDevice->GetModuleInfo().pageSize];
    RV rv =
        flashDevice->ReadPage(pba.blockID, pba.pageID, _buffer, offset, size);
...
RV FTL01::WritePage(LBA lba, const BYTE *buffer, int offset, size_t size) {
  ASSERT(flashDevice);
  ASSERT(buffer);
  ...
  if (ps == LIVE) {  // allocate new page
    ...
    pba = TranslateLBAtoPBA(lba);  // AllocNewPage operation may change the pba
                                   // value, fetch it again
...
PBA FTL01::TranslateLBAtoPBA(LBA lba) {
  ...
  if (pba.blockID != -1 && pba.pageID != -1) {
    ASSERT(reverseMapList[pba.blockID *
                              flashDevice->GetModuleInfo().pageCountPerBlock +
                          pba.pageID] == lba);
  }
...
RV FTL01::RegisterEntry(LBA lba, PBA pba) {
  ASSERT(lba >= 0 && lba < info.mapListSize);  // ASSERT
...
PBA FTL01::AllocNewPage(void) {
   ...
   * If there is free page in current block(the last block in dirtyList), then
   * allocate the page, otherwise, allocate a new page in freeList. If there is
   * no free block in freeList, then, a garbage collection will be activated.
   */
  PBA pba;
  int pageCountPerBlock = flashDevice->GetModuleInfo().pageCountPerBlock;
  ...
  if (dirtyList.size() == 0) {     // dirtyList is empty, scan freeList
    ...
    if (_pos >= (int)dirtyList.size()) {  // No page free in dirtyList (allc,
                                          // live, dead), scan freeList
      if (freeList.size() ==
          0) {  // no free page in freeList, active garbage collection
        RV rv = ReclaimBlock();
...
RV FTL01::ReclaimBlock(void) {
  ...
  if (deadList.size() > 0) {
    ...
    for (vector<BLOCK_ID>::iterator it = deadList.begin(); it != deadList.end();
         ++it) {
      // erase all blocks
  ...
  for (vector<BLOCK_ID>::iterator it = dirtyList.begin(); it != dirtyList.end();
       ++it) {
  ...
  // Wear leveling
  if (pCounter->GetEraseCount(mostDirtyBlockID) - leastEraseCount >
      info.wearLevelingThreshold) {
    // Just do it
    int index = 0;
...
void FTL01::SetPageState(PBA pba, PAGE_STATE ps) {
  ASSERT(blockState);
  ...
  switch (ps) {
    ...
    case DEAD:
      blockState[pba.blockID].deadPages++;
      if (blockState[pba.blockID].deadPages ==
          flashDevice->GetModuleInfo().pageCountPerBlock) {
